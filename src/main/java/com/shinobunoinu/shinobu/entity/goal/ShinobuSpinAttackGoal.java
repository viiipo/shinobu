...
public class ShinobuSpinAttackGoal extends ShinobuAttackGoal {
    protected final ShinobuEntity mob;
    protected float range;
    protected float applyKnockback = 1;
    private int animationTicker = 20;

    public ShinobuSpinAttackGoal(ShinobuEntity shinobu, Mode mode, float range) {
        super(shinobu, mode);
        this.mob = shinobu;
        this.range = range;
    }
    ...
    @Override
    public void tick() {
        LivingEntity entityTarget = this.mob.getTarget();
        if (entityTarget != null) {
            // 优先检查目标是否进入攻击范围
            double targetDistanceSqr = mob.distanceToSqr(entityTarget);
            ...
            if (targetInRange && this.mob.getSpinAttackAnimTimer() == 0) {
                ...
                animationTicker = this.mob.SPIN_ATTACK_ANIMATION_DURATION;
            }
            ...
            for (LivingEntity entityHit : entitiesHit) {
                if (entityHit == mob) continue;
                double dx = entityHit.getX() - mob.getX();
                double dz = entityHit.getZ() - mob.getZ();
                double distanceSqr = dx*dx + dz*dz;
                if (distanceSqr <= range * range) {
                    if (mob.getSpinAttackAnimTimer() == 55 || mob.getSpinAttackAnimTimer() == 45 || mob.getSpinAttackAnimTimer() == 30 || mob.getSpinAttackAnimTimer() == 15 || mob.getSpinAttackAnimTimer() == 10 || mob.getSpinAttackAnimTimer() == 5) {
