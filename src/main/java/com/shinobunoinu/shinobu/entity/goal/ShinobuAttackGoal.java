...
// 优先判断攻击 Shinobu 自己的生物（非主人）
        ShinobuEntity shinobu = (ShinobuEntity) this.mob;
        LivingEntity attacker = shinobu.getLastHurtByMob();
        if (attacker != null
                && !attacker.getUUID().equals(shinobu.getOwnerUUID())
                && shinobu.getLastHurtByMobTimestamp() != this.lastEventTimestamp) {

            this.cachedTarget = attacker;
            this.lastEventTimestamp = shinobu.getLastHurtByMobTimestamp();
            return true;
        }
